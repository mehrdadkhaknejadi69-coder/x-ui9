...
DEFAULT_ADMIN_USERNAME = os.environ.get("ADMIN_USERNAME", "VodiAdmin").strip() or "VodiAdmin"
...
async def create_inbound_client(uid: str, request: Request, _=Depends(require_auth)):
    ...
    async with LINKS_LOCK:
        ...
        if not parent:
            raise HTTPException(status_code=404, detail="اینباند پیدا نشد")
        source = dict(parent)
    label = str(body.get("label") or f"Client · {len([x for x in LINKS.values() if x.get('parent_inbound_id') == uid])+1}").strip()[:120]
...
async def subscription_portal(uuid: str, request: Request):
    ...
    safe={"label":escape_html(label),"protocol":escape_html(protocol),"raw":escape_html(raw_url),"info":escape_html(info_url),"uuid":escape_html(uuid),"remaining":escape_html(remaining),"expires":escape_html(expires[:19]),"status":"فعال" if active else "غیرفعال","pct":str(pct),"used":escape_html(fmt_bytes(used)),"limit":escape_html(fmt_bytes(limit) if limit else "نامحدود"),"ip":str(ip_limit or 0),"conn":str(conn_limit or 0)}
